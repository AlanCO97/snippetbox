...
func (m *UserModel) Insert(name, email, password string) error {
	...
	_, err = stmt.ExecContext(ctx, name, email, string(hashedPassword))
	if err != nil {
...
func (m *UserModel) Authenticate(email, password string) (int, error) {
	var (
		...
		query          string          = `SELECT id, hashed_password FROM users WHERE email = ?`
	)
	...
	err = stmt.QueryRowContext(ctx, email).Scan(&id, &hashedPassword)
	if err != nil {
